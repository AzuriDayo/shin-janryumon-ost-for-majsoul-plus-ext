if (!!view && !!uiscript) {
    ...
    const newRound = (roundInfo) => {
        ...
        // ben = round# of east/south
        // ben 4 even after east 3 repeat

        // chang is the riichi stick counter
        // ben is the 8 black dots counter

        // ju is round counter, starts at 0 goes to 3
        // chang 0 = east
        // chang 1 = south

        // Start of shin janryumon logic
        const E_PREVAILING_WIND = {
            EAST: 0,
            SOUTH: 1,
        };
        let prevailingWind = roundInfo.chang; // east=0 south=1
        if (prevailingWind === undefined) prevailingWind = E_PREVAILING_WIND.EAST;
        let roundNumber = roundInfo.ju; // 0-3 (+1 for actual)
        if (roundNumber === undefined) roundNumber = 0;
        let ostNum = 1 + roundNumber;
        if (prevailingWind === E_PREVAILING_WIND.SOUTH) {
            ostNum += 4; // ost #5-8
        }

        // Missing logic, if gamemode is east-only, but into south 1-4
        // Should always force ostNum = 8

        /*
        if someone can fix gameN 2-4, 6-8, much appreciated
        broken ostNum 2,3,4,6,7,8
        */

        thisTurnBGM = 'game' + ostNum
        richedCount = 0
        fewPai = false
